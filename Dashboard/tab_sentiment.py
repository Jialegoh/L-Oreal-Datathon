...
def render_sentiment_tab(filtered_df, apply_brand_style):
    ...
    # Sentiment Distribution by Cluster Category (Stacked Bar Chart)
    st.markdown('<h4 style="color:#000;">Sentiment Distribution by Cluster Category</h4>', unsafe_allow_html=True)
    
    # Check if category/cluster column exists
    category_col = None
    possible_category_cols = ['new_cluster', 'cluster', 'cluster_category', 'topic', 'product_category', 'sub_category']
    ...
    if category_col:
        try:
            ...
            category_sentiment = filtered_df.groupby([category_col, 'sentiment']).size().reset_index(name='count')
            
            # Get top 10 categories by total count
            top_categories = (category_sentiment.groupby(category_col)['count']
                            .sum()
                            .sort_values(ascending=False)
                            .head(10)
                            .index.tolist())
            
            # Filter data to only include top 10 categories
            category_sentiment_top10 = category_sentiment[category_sentiment[category_col].isin(top_categories)]
            
            # Create stacked bar chart
            fig_stacked = px.bar(
                category_sentiment_top10, 
                x=category_col, 
                y='count', 
                color='sentiment',
                title="Sentiment Distribution by Cluster Category (Top 10)",
                labels={
                ...
                color_discrete_map={
                    'positive': '#2563eb',    # Blue for positive
                    'negative': '#dc2626',    # Red for negative  
                    'neutral': '#f87171',     # Light coral for neutral
                    'pos': '#2563eb',         # Alternative positive
                    'neg': '#dc2626',         # Alternative negative
                    'neu': '#f87171'          # Alternative neutral
                },
                category_orders={category_col: top_categories}  # Order x-axis by descending total count
            )
            
            # Apply brand styling and customize layout
            apply_brand_style(fig_stacked)
            ...
            st.warning(f"Could not create category sentiment chart: {str(e)}")
    else:
        st.info(f"💡 Category column not found. Available columns: {list(filtered_df.columns)}")
        st.info("To display the Sentiment Distribution by Category chart, please ensure your dataset contains one of these columns: category, cluster, cluster_category, topic, product_category, or sub_category.")
